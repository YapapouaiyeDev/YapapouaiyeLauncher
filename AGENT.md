...
**Yapapouaiye Launcher** is a custom Electron-based launcher for a modded Minecraft server (version 1.21.1 with NeoForge). It provides a polished UI for players to authenticate, configure game settings, sync mods from a remote Gitea repository, and launch the game.
...
├── .env                 # Environment variables (GH_TOKEN, GITEA_*)
...
- `mods/` — Synced server mods (from Gitea)
...
GITEA_OWNER=hannyvan26
GITEA_REPO=ModYapapouaiyeLauncher
GITEA_BRANCH=main
...
| `syncMods()` | Synchronize mods from Gitea |
...
- **Mod Sync:** Compares SHA hashes between local mods and the Gitea repository; only downloads changed mods
